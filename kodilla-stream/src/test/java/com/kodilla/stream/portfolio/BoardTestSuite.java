...
class BoardTestSuite {
    ...
    void testAddTaskListFindOutdatedTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()          // [4]
                ...
                .filter(t -> t.getDeadline().isBefore(now()))  // [7]
                .collect(toList());                                      // [8]
    ...
    void testAddTaskListFindLongTasks() {
        ...
        long longTasks = project.getTaskLists().stream()                  // [3]
                ...
                .filter(d -> d.compareTo(now().minusDays(10)) <= 0)  // [7]
                .count();                                                      // [8]
    ...
    Board project = prepareTestData();
        ...
        int liczbaElementow = (int) project.getTaskLists().stream().filter(inProgressList::contains)
                .flatMap(n -> n.getTasks().stream())
                        .count();

        int sumaElementow = (int) project.getTaskLists().stream().filter(inProgressList::contains)
                .flatMap(n -> n.getTasks().stream()).mapToInt(task -> Period.between(task.getCreated(), LocalDate.now()).getDays()).sum();


        double result = sumaElementow / liczbaElementow;
        assertEquals(3, liczbaElementow);
        assertEquals(30, sumaElementow);
